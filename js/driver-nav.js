...
filters.addEventListener("change", function(evt){
      ...
      //filter only
      if (type) {
        filterResult = filteredItems = items.filter(el => el.querySelector('td:nth-child(1)').innerHTML.indexOf(type) > -1)
        ...
        if(keyword){
          ...
          || el.querySelector('td:nth-child(6)').innerHTML.toUpperCase().indexOf(keyword) > -1 
...
function setHTML(items) {
  ...
  //next button
  paginationHTML += `<button ${currentPage === endPage && 'disabled'} class="${currentPage === endPage ? 'cursor-not-allowed' : 'next'} relative inline-flex items-center px-3 py-2 mx-1 shadow rounded rounded-r-md border border-primary bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
